...
export default function DashboardPage() {
  ...
  return (
    <DashboardLayout userName="Renjiro" userRole="Premium User">
      ...
      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 px-4 sm:px-6 md:px-8 mb-6 sm:mb-8">
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-6 sm:pb-8">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 sm:p-5 border-b border-gray-100">
            ...
            <button
              onClick={() => setModalOpen(true)}
              className="text-xs font-medium bg-blue-50 text-blue-600 hover:bg-blue-100 px-3 py-1.5 rounded-xl transition-colors text-left sm:text-center"
            >
              + Add Expense
            </button>
          </div>
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-xs font-medium text-gray-500 px-3 sm:px-5 py-3">Expense</th>
                  <th className="hidden sm:table-cell text-left text-xs font-medium text-gray-500 px-3 sm:px-5 py-3">Category</th>
                  <th className="hidden md:table-cell text-left text-xs font-medium text-gray-500 px-3 sm:px-5 py-3">Paid By</th>
                  <th className="text-right text-xs font-medium text-gray-500 px-3 sm:px-5 py-3">Amount</th>
                </tr>
              </thead>
              <tbody>
                {expenses.slice(0, 6).map((expense) => (
                  <tr key={expense.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-3 sm:px-5 py-3 font-medium text-gray-900 text-xs sm:text-sm">{expense.title}</td>
                    ...
                    <td className="hidden md:table-cell px-3 sm:px-5 py-3 text-gray-600 text-xs sm:text-sm">{expense.paidBy}</td>
                    <td className="px-3 sm:px-5 py-3 text-right font-semibold text-gray-900 text-xs sm:text-sm">${expense.amount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
